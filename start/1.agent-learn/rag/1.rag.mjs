import "dotenv/config"; // 为什么要放在最前面？因为有些模块可能会在导入时读取环境变量，如果不先加载环境变量，可能会导致错误。
...
// 使用嵌入模型将文档向量化，存入向量数据库中
const vectorStore = await MemoryVectorStore.fromDocuments(documents, embeddings);

// 创建一个检索器，设置返回最相关的3条文档 （余弦相似度最大的 3 个 Doc）
...
for (const q of questions) {
  ...
  const context = retrievedDocs.map((doc, index) => `[片段${index + 1}]\n${doc.pageContent}`).join("\n------\n");
